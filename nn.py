...
def get_reseller_markup():
    ...
    markup = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    buttons = [
        KeyboardButton("➕ Add Reseller"),
        KeyboardButton("➖ Remove Reseller"),
        KeyboardButton("📋 List Resellers"),
        KeyboardButton("💰 Reseller Balance"),
...
def get_admin_markup():
    ...
    buttons = [
        ...
        KeyboardButton("⚙️ Max Time"),
        KeyboardButton("👔 Reseller Tools"),  # New reseller button
        KeyboardButton("⬅️ Back")
    ]
    markup.add(*buttons)
    return markup
...
def welcome_start(message):
    ...
    welcome_text = (
        ...
        "📌 *Try This Command:*\n"
        "🚀 *Start an attack!*\n\n"
        "👑 *BOT CREATED BY:* [@LASTWISHES0](https://t.me/LASTWISHES0)"
    )
    ...
    keyboard.add(InlineKeyboardButton("👑 CREATOR", url="https://t.me/LASTWISHES0"))
    keyboard.add(InlineKeyboardButton("💬 DM FOR REBRANDING", url="https://t.me/LASTWISHES0"))
...
def process_stylish_broadcast(message):
    ...
    keyboard.add(
        InlineKeyboardButton("👑 𝗖𝗥𝗘𝗔𝗧𝗢𝗥", url="https://t.me/LASTWISHES0")
    )
...
@bot.message_handler(func=lambda m: m.text == "🧵 Show Threads")
def show_threads(message):
    ...
    if is_admin_user:
        response = (
            f"📊 *Current Thread Configuration (Admin View)*\n\n"
        ...
        response = (
            f"📊 *Current Bot Power*\n\n"
            f"🖥️ Active Servers: `{len(active_vps)}`\n"
            f"⚡ Total Power: `{total_power} threads`\n\n"
            f"▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰\n"
            f"👑 Powered by APNA BHAI"
        )
...
@bot.message_handler(func=lambda message: message.text == "👔 Reseller Tools" and is_admin(message.from_user.id))
...
@bot.message_handler(func=lambda message: message.text == "➕ Add Reseller" and is_admin(message.from_user.id))
...
@bot.message_handler(func=lambda message: message.text == "➖ Remove Reseller" and is_admin(message.from_user.id))
...
@bot.message_handler(func=lambda message: message.text == "📋 List Resellers" and is_admin(message.from_user.id))
def list_resellers_command(message):
    ...
    for reseller_id, details in reseller_data['resellers'].items():
        ...
        response += (
            f"🆔 ID: `{reseller_id}`\n"
            f"👤 Username: {username}\n"
            f"💰 Balance: {details['balance']} Rs\n"
            f"📅 Added: {datetime.fromisoformat(details['added_at']).strftime('%Y-%m-%d')}\n"
...
@bot.message_handler(func=lambda message: message.text == "💰 Reseller Balance" and is_admin(message.from_user.id))
...
@bot.message_handler(func=lambda message: message.text == "⏱️ Bot Uptime")
